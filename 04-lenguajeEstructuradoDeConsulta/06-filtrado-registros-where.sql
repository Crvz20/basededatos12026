...
ORDER BY p.id_categoria;
    ...
    --comienza 
    --WHERE nombre LINE 'Cliente%'
    --Valores que comienza con clientes

    --Termina
    --WHERE correo LINE '%mail.com'
    --valores que terminen con mail.com

    --contiene
    -- Where nombre '%a%
    --valores que contienen el caracter "a" en cualquier posición.

    --Buscar codigoss de productos que comienzen con: P001
    ...
    -- Buscar los correor de los clientes que termine exactamente con 10gmail.com
    select 
    c.id_cliente,
    c.nombre,
    c.correo
    FROM clientes AS c
    WHERE c.correo LIKE '%10gmail.com';

-- Muestra los nombres de los productos que contengan el caracter 1
...
--el guión bajo representa exactamente un caracter
...
--Mostrar los codigos con P000 y exactamente un caracter adicional 
  ...
  select 
    ...
    --Busca un guión bajo literal
    --en LIKE _ es un comidin
    --Las ciudades en esta base de datos cintiene guiones bajos, por ejemplo:

    --Ciudad_1_1
    --para buscar un guión bajo literal mediante corchetes se puede utilizar lo siguiente
    SELECT*
    FROM ciudades
    WHERE nombre LIKE '%[_]%' --Esto de aqui significa un caracter de guion bajo literal